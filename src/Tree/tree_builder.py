...
def _get_children_nodes_chance_node(parent_node, limit_to_street):
...
def _get_children_player_node(parent_node):
    ...
    # 3.0 bet actions
    possible_bets = _get_possible_bets(parent_node) # [N,P], P=2
...
def _get_children_nodes(parent_node, limit_to_street):
    ...
    if parent_node.terminal:
        ...
        return _get_children_nodes_chance_node(parent_node, limit_to_street)
    else: # inner nodes -> handle bet sizes
        return _get_children_player_node(parent_node)
    assert(False)


def _build_tree_dfs(current_node, limit_to_street):
...
def build_tree(params,limit_to_street=True):
    ''' Builds the tree
    ...
    _build_tree_dfs(root, limit_to_street)
